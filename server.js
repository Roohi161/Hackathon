...
const DELETE_HACKATHON = async (req, res) => {
  try {
    const result = await pool.query('DELETE FROM hackathons WHERE id = $1 RETURNING id', [req.params.id]);
    if (result.rows.length === 0) return res.status(404).json({ error: 'Hackathon not found' });
    res.json({ success: true, data: result.rows[0] });
  } catch (error) {
    res.status(500).json({ error: error.message });
...
app.delete('/api/v1/hackathons/:id', DELETE_HACKATHON);
...
app.delete('/hackathons/:id', DELETE_HACKATHON);
